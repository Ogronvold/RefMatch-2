    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    setupLabel(versionLabel, "0.4.2  |  STREAM MODE", 10.5f, juce::Colour(cyan), true, juce::Justification::centredRight);
    ...
    switchButton.setTooltip("20 ms MIX fade. External playback is controlled in its source app; pause it before returning to MIX.");
...
{
    if (audioProcessor.isReferenceSelected())
        audioProcessor.setReferenceSelected(false);
    else if (!audioProcessor.isReferenceCaptureRunning())
...
void RefMatchAudioProcessorEditor::updateSpotifyInfo()
{
    ...
    if (!spotifyControlReady || spotifyError || spotify.isBusy()) return;
...
void RefMatchAudioProcessorEditor::updateSourceState()
{
    ...
    const bool busy = spotify.isBusy();
    listeningLabel.setText(ref ? "NOW: SYSTEM REF" : "NOW: YOUR MIX", juce::dontSendNotification);
    listeningLabel.setColour(juce::Label::textColourId, juce::Colour(ref ? violet : cyan));
    switchButton.setButtonText(ref ? "SWITCH TO MIX" : audioProcessor.isReferenceCaptureRunning() ? "SWITCH TO REF" : "ENABLE SYSTEM AUDIO");
...
void RefMatchAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.setColour(juce::Colour(muted));
    g.setFont(juce::Font(juce::FontOptions(10.0f)));
    g.drawText("Pause external playback before returning to MIX", side.getX() + 18.0f, 618.0f,
