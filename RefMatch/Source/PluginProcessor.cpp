...
void RefMatchAudioProcessor::switchWithSpotify()
{
    ...
    if (isReferenceSelected() || transportSwitch.pending()) {
        ...
        if (transportSwitch.getPhase() == Phase::pausing) return;
        if (spotifyController.isBusy()) {
            setReferenceSelected(false);
            transportSwitch.requestMix(); // Timer sends PAUSE after status returns.
            return;
        }
        pauseSpotifyAndRestore();
        return;
    }
    transportError.clear();
    if (spotifyController.isBusy()) { transportError = "Spotify is busy. Try SWITCH again."; return; }
    if (!isReferenceCaptureRunning()) { startReferenceCapture(); return; }
    transportSwitch.beginReference();
    setReferenceSelected(true);
    fadeDeadline = juce::Time::getMillisecondCounterHiRes() + 1000.0;
}

void RefMatchAudioProcessor::pauseSpotifyAndRestore()
{
    transportSwitch.requestMix();
    spotifyController.request(SpotifyDesktopController::Command::pause,
        [this](bool ok, SpotifyDesktopInfo, juce::String error) {
            setReferenceSelected(false);
            spotifyStarted = false;
            transportSwitch.reset();
            if (!ok) transportError = error + " Pause Spotify manually if it is still audible.";
        });
}
...
{
    using Phase = TransportSwitch::Phase;
    if ((hasReferenceFailure() || !isReferenceCaptureRunning()) && isReferenceSelected()) {
        setReferenceSelected(false);
        transportError = "Capture failed. MIX restored.";
        if (transportSwitch.getPhase() == Phase::playing) transportSwitch.requestMix();
        else if (spotifyStarted) transportSwitch.requestMix();
        else transportSwitch.reset();
    }
    if (!isReferenceSelected() && spotifyStarted && !transportSwitch.pending())
        transportSwitch.requestMix();
    if (transportSwitch.getPhase() == Phase::pausing && !spotifyController.isBusy()) {
        pauseSpotifyAndRestore();
        return;
    }
    ...
    if (juce::Time::getMillisecondCounterHiRes() > fadeDeadline) {
        setReferenceSelected(false);
        transportSwitch.reset();
        transportError = "MIX restored: Logic did not finish the fade. Start Logic playback and retry.";
        return;
    }
    if (!transportSwitch.fadeComplete(isMixMuted())) return;
    spotifyController.request(SpotifyDesktopController::Command::play,
        [this](bool ok, SpotifyDesktopInfo, juce::String error) {
            const bool keepReference = transportSwitch.playComplete(ok,
                isReferenceSelected() && !hasReferenceFailure() && isReferenceCaptureRunning());
            spotifyStarted = ok;
            if (!keepReference) {
                setReferenceSelected(false);
...
void RefMatchAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    referenceSafety.reset();
    effectiveReference.store(false);
    referenceFade.prepare(sampleRate, false);
}
...
void RefMatchAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    if (rearmReference.exchange(false)) referenceSafety.reset();
    const bool reference = referenceSafety.update(
        apvts.getRawParameterValue("reference")->load() > 0.5f,
        referenceCapture.isRunning(), gotReference, numSamples, currentSampleRate);
    effectiveReference.store(reference);
    referenceFailure.store(apvts.getRawParameterValue("reference")->load() > 0.5f && !reference);
