...
RefMatchAudioProcessorEditor::RefMatchAudioProcessorEditor(RefMatchAudioProcessor& p)
    : AudioProcessorEditor(&p), audioProcessor(p), spotify(p.getMediaController())
{
    ...
    setupLabel(versionLabel, "0.4.4  |  STREAM MODE", 10.5f, juce::Colour(cyan), true, juce::Justification::centredRight);
    setupLabel(straplineLabel, "LISTEN. COMPARE. MATCH.", 10.5f, juce::Colour(muted));
    setupLabel(trackLabel, "Choose a media source", 19.0f, juce::Colour(text), true);
    setupLabel(artistLabel, "Spotify, Music or browser", 12.0f, juce::Colour(muted));
    setupLabel(spotifyStateLabel, "PLAY SOURCE ONCE, THEN USE SWITCH", 10.5f, juce::Colour(amber), true);
    ...
    enableSpotifyButton.onClick = [this] { audioProcessor.startReferenceCapture(); };
    ...
    playPauseButton.onClick = [this] { runSpotifyCommand(SystemMediaController::Command::playPause); };
    prevButton.onClick = [this] { runSpotifyCommand(SystemMediaController::Command::previous); };
    nextButton.onClick = [this] { runSpotifyCommand(SystemMediaController::Command::next); };
    enableSpotifyButton.setTooltip("Enable optional reference meters. Not required for SWITCH.");
    permissionButton.setTooltip("Open macOS Screen Recording settings for reference meters.");
    switchButton.setTooltip("Mute MIX and send system PLAY. Return sends system PAUSE and restores MIX.");
    ...
    runSpotifyCommand(SystemMediaController::Command::status);
    updateSourceState();
    startTimerHz(20);
} 
...
void RefMatchAudioProcessorEditor::enableSpotifyControl()
{
    ...
    spotifyError = false;
    spotifyStateLabel.setText("CHECKING SYSTEM MEDIA...", juce::dontSendNotification);
    runSpotifyCommand(SystemMediaController::Command::authorise);
}
...
void RefMatchAudioProcessorEditor::applySpotifyInfo(const SystemMediaInfo& info)
{
    spotifyInfo = info;
    spotifyControlReady = info.authorised && !spotifyError;
    trackLabel.setText(info.track.isNotEmpty() ? info.track : "Choose a media source", juce::dontSendNotification);
    artistLabel.setText(info.artist.isNotEmpty() ? info.artist : "Spotify, Music or browser", juce::dontSendNotification);
    trackLabel.setTooltip(info.track);
    artistLabel.setTooltip(info.artist);
    playPauseButton.setButtonText("PLAY / PAUSE");
    enableSpotifyButton.setButtonText("ENABLE METERS");
    if (!spotifyError)
    {
        spotifyStateLabel.setText(info.authorised ? "SYSTEM MEDIA CONTROL AVAILABLE" : "PLAY SOURCE ONCE, THEN USE SWITCH", juce::dontSendNotification);
        spotifyStateLabel.setColour(juce::Label::textColourId, juce::Colour(info.authorised ? green : muted));
    }
}

void RefMatchAudioProcessorEditor::runSpotifyCommand(SystemMediaController::Command command)
{
    if (spotify.isBusy() || audioProcessor.isTransportPending()) return;
    juce::Component::SafePointer<RefMatchAudioProcessorEditor> safe(this);
    spotify.request(command, [safe](bool ok, SystemMediaInfo info, juce::String error)
    {
        if (!safe) return;
...
{
    audioProcessor.switchWithSystemMedia();
    updateSourceState();
}
...
void RefMatchAudioProcessorEditor::updateSourceState()
{
    ...
    switchButton.setButtonText(audioProcessor.isTransportPending() ? "RETURN TO MIX" : ref ? "SWITCH TO MIX" : "SWITCH TO REF");
...
void RefMatchAudioProcessorEditor::timerCallback()
{
    ...
    captureStateLabel.setText(audioProcessor.hasReferenceFailure() ? "MIX RESTORED - CHECK REFERENCE"
                              ...
                              : "METERS OFF - ENABLE METERS", juce::dontSendNotification);
...
void RefMatchAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    drawPill(g, { side.getX() + 18.0f, side.getY() + 16.0f, 104.0f, 24.0f }, "SYSTEM MEDIA", juce::Colour(green), true);
    ...
    g.setColour(juce::Colour(muted));
    g.setFont(juce::Font(juce::FontOptions(10.0f)));
    g.drawText("SWITCH controls the active media player", side.getX() + 18.0f, 686.0f,
