...
void MatchEQ::refresh()
{
    ...
    const auto gains=EQDesign::scaled(getGains(),amount.load(),limit.load(),sr);
    std::array<EQDesign::Coeff,EQDesign::bands> coeff{};
    for(int b=0;b<EQDesign::bands;++b)coeff[b]=EQDesign::peak(sr,EQDesign::centre(b),gains[b]);
    const juce::SpinLock::ScopedLockType guard(lock);published=coeff;dirty=true;
}
void MatchEQ::process(juce::AudioBuffer<float>& buffer)
{
    ...
    for(int i=0;i<buffer.getNumSamples();++i) {
        if(rampRemaining>0) {
            ...
            for(int b=0;b<EQDesign::bands;++b) {
...
{
    const auto sr=rate.load();const auto gains=EQDesign::scaled(getGains(),amount.load(),limit.load(),sr);
