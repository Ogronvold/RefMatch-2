    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    setupLabel(versionLabel, "0.4.1  |  STREAM MODE", 10.5f, juce::Colour(cyan), true, juce::Justification::centredRight);
    ...
    setupLabel(spotifyStateLabel, "CONNECT SPOTIFY TO START", 10.5f, juce::Colour(amber), true);
    ...
    enableSpotifyButton.setTooltip("Grant RefMatch Control permission to control Spotify. Logic's permissions are separate.");
    permissionButton.setTooltip("Open macOS Automation settings if Spotify control was denied.");
    switchButton.setTooltip("Fade your mix out, then start Spotify. Return to MIX pauses Spotify and restores your mix.");
    captureStateLabel.setTooltip("System audio is used for measurement only. Spotify plays directly to your audio device.");

    updateSourceState();
    startTimerHz(20);
    // Recover an existing grant when the editor is reopened; never prompt here.
...
{
    stopTimer();
    if (switchState.pending()) audioProcessor.setReferenceSelected(false);
    setLookAndFeel(nullptr);
}

void RefMatchAudioProcessorEditor::enableSpotifyControl()
{
    if (spotify.isBusy() || switchState.pending()) return;
    spotifyError = false;
...
{
    spotifyInfo = info;
    spotifyControlReady = info.authorised;
    ...
    enableSpotifyButton.setButtonText(info.authorised ? "RECONNECT" : "CONNECT SPOTIFY");
    if (!spotifyError)
    {
        spotifyStateLabel.setText(info.authorised ? "SPOTIFY CONNECTED" : "CONNECT SPOTIFY TO START", juce::dontSendNotification);
        spotifyStateLabel.setColour(juce::Label::textColourId, juce::Colour(info.authorised ? green : muted));
    }
}

void RefMatchAudioProcessorEditor::runSpotifyCommand(SpotifyDesktopController::Command command, bool switching)
{
    if (spotify.isBusy()) return;
    juce::Component::SafePointer<RefMatchAudioProcessorEditor> safe(this);
    spotify.request(command, [safe, switching](bool ok, SpotifyDesktopInfo info, juce::String error)
    {
        if (!safe) return;
        safe->spotifyError = !ok;
        safe->applySpotifyInfo(info);
        if (switching)
            safe->audioProcessor.setReferenceSelected(safe->switchState.complete(ok));
...
{
    if (spotify.isBusy() || !switchState.begin(audioProcessor.isReferenceSelected(), spotifyControlReady)) return;
    spotifyError = false;
    if (switchState.getPhase() == SwitchState::Phase::pausingSpotify)
    {
        runSpotifyCommand(SpotifyDesktopController::Command::pause, true);
    }
    else
    {
        ...
        // Mute before starting the external source, not after it is audible.
        audioProcessor.setReferenceSelected(true);
        fadeStarted = juce::Time::getMillisecondCounterHiRes();
    }
    updateSourceState();
}
...
void RefMatchAudioProcessorEditor::updateSpotifyInfo()
{
    ...
    if (!spotifyControlReady || spotifyError || spotify.isBusy() || switchState.pending()) return;
    runSpotifyCommand(SpotifyDesktopController::Command::status);
}
...
void RefMatchAudioProcessorEditor::updateSourceState()
{
    ...
    const bool pending = switchState.pending();
    const bool busy = spotify.isBusy() || pending;
    listeningLabel.setText(pending ? "SWITCHING..." : (ref ? "NOW: SPOTIFY REF" : "NOW: YOUR MIX"), juce::dontSendNotification);
    listeningLabel.setColour(juce::Label::textColourId, juce::Colour(ref ? violet : cyan));
    switchButton.setButtonText(pending ? "SWITCHING..." : (ref ? "SWITCH TO MIX" : "SWITCH TO REF"));
    switchButton.setEnabled(!busy && (ref || spotifyControlReady));
...
{
    ++timerTicks;
    if (switchState.fadeReady(audioProcessor.isMixMuted(),
                              juce::Time::getMillisecondCounterHiRes() - fadeStarted > 120.0))
        runSpotifyCommand(SpotifyDesktopController::Command::play, true);

    if (timerTicks % 20 == 0)
        updateSpotifyInfo();

    const auto captureStatus = audioProcessor.getReferenceCaptureStatus();
    captureStateLabel.setText(audioProcessor.isReferenceCaptureRunning() ? "SYSTEM AUDIO READY"
                              : audioProcessor.isReferenceCaptureStarting() ? "STARTING AUDIO CAPTURE..."
                              : "SYSTEM AUDIO OFF - SWITCH TO START", juce::dontSendNotification);
